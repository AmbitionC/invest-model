...
提案（P1-P18，`docs/model_change_proposals.md`）→ 预登记验证（E1-E14，`scripts/validation/`+提案文档，判据写死勿改）→ **高置信直升生产（多市场/多周期样本外过预登记判据且有安全边际，跳过影子；否则走影子）** → 保留红线回退。晋升路径 2026-07-13 治理修订（用户定：置信度高就别走影子、别拿真金白银慢慢试），四条判定见 model_change_proposals「高置信直升」节。
...
市况自适应（UP/RANGE/DOWN 标签切换整套买卖策略）2026-07-15 判据 0/8+2/8 未过、不登记 P19（"不接飞刀"8/8 成立但已由 MA60 前置闸内化；详见 strategy_research_202607 附四）。
