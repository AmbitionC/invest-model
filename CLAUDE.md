...
**P17（美股期权层·负 Gamma×到期 挤压探针 US-O5）+ E13** 已登记待实现（源自 0713 投顾笔记；判据写死于 `scripts/validation/e13_gamma_squeeze.py`）：**代理法**（无真实 dealer Gamma/GEX 数据，用 临近到期+VIX 骤升+破 MA10 近似），E13 证不出显著恶化则不上线——仅美股模块、A 股链路零改动。
