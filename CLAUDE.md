...
**P17（美股期权层·负 Gamma×到期 挤压探针 US-O5）+ E13** 已实现·overlay 默认 off（源自 0713 投顾笔记；判据写死于 `scripts/validation/e13_gamma_squeeze.py`，生产探测同用 `us/signals.py:gamma_squeeze_now`）：**代理法**（无真实 dealer Gamma/GEX 数据，用 临近月度 OpEx+VIX 骤升+破 MA10 近似）。E13 跑在 `us-update.yml` 的 `e13` 模式（bump `ops/us-invoke.trigger=e13`）；**PASS+高置信直升才把 `US_GAMMA_OVERLAY` 切 strict，FAIL/样本不足永久 off**——仅美股模块、A 股链路零改动，判据未跑出前卖 put 行为不变。
