...
def _quality_cell(mr) -> str:
    """模型质量参谋：rank_pct(全市场因子分位)→"前X%" + 认同(✓,≥70分位)/存疑(⚠,<50分位)。"""
    if mr is None:
        return "—"
    try:
        v = float(mr)
    ...
    if not np.isfinite(v):
        return "—"
    top = (1.0 - v) * 100.0
    mark = "✓" if v >= 0.7 else ("⚠" if v < 0.5 else "")
    return f"前{top:.0f}%{mark}"


def _table(lines: list[str], rows: list[dict]) -> None:
    lines.append("| 代码 | 名称 | 动作 | 现权重→目标 | 约股数 | 买点/挂单价 | 理由 | 止损价 | 现价 | 分级 | 模型分位 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|---|")
    for r in rows:
        ...
        lines.append(
            ...
            f"{_quality_cell(r.get('model_rank'))} |")
...
def render_markdown(plan: ActionPlan) -> str:
    ...
    lines.append(
        "- 模型分位=该票多因子分在全市场的位置（前X%，越小越强）；✓=模型认同(≥前30%)、"
        "⚠=模型存疑(低于中位)。**标的由投顾定，模型只做质量参谋+时机+风控，不选股。**")
