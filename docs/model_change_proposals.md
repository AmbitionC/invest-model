...
- **落地（2026-07-13，用户定"够用就上"）：提示版已进生产**——`invest_model/signals/top_feature.py`
  `top_feature_now`（判据同回测预登记常量）接入 `action_plan.py` 计划层 hints，输出"顶部特征预警"行
  （持仓浮盈达标+波动骤放大+放量→提示主动兑现），**只提示、不自动减仓、不改权重**（不触碰 risk.py 自动逻辑）。
  这条不受 E12 约束（非自动交易）；**自动减仓**仍须待重新预登记的 E12 过关才谈。测试 `tests/test_top_feature.py`。
