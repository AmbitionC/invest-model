...
**状态：预登记 + 实现完成（本次提交）——`market_timing.MarketTiming` 增 `ma60_gate`
开关（**默认 False=现状逐字一致**，生产零改动）+ `ma60_gate_active` 纯函数 +
`scripts/validation/e9_index_ma60.py`（已接入 run_all，E9 段随周度验证自动跑）+
`tests/test_ma60_gate.py` 4 组回归。E9 真实数据过关（判据预登记在脚本头）后，
开关置 True 进影子。**
