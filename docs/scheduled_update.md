...
## 统一 FaaS 定时调度（✅ 已实施：全部定时任务迁至阿里云 FC）

2026-07 起，**所有**依赖 GitHub Actions schedule 的定时任务（盯盘/复盘/操作计划/
数据更新/ETF 入库/快照提醒）统一迁到阿里云函数计算，理由：Actions 免费额度不足
（盯盘一项每交易日 ~320 分钟）+ schedule 实测延迟 2.8~5.8 小时且有整档跳过。

### 架构

一份代码包（`bash faas/package.sh` 产出），两个互相独立的 FC 函数，
资源配置在 **fe-journey-faas 仓库的 `s.yaml`**（与 fe-journey 的 Web 函数完全解耦，
仅共用阿里云账号/地域/部署流水线；push master 自动 `s deploy`）：

| FC 函数 | 定时触发器 (UTC / 北京) | payload → job | 对应原 workflow |
|---|---|---|---|
| invest-live-watch (512MB/120s) | `0 0/3 1-7 ? * MON-FRI`（每 3 分钟 09:00-15:59） | `live_watch` | live-watch + 看门狗 |
| invest-scheduler (4GB/3h) | `0 20 7 ? * MON-FRI`（15:20） | `snapshot_remind` | snapshot-remind |
| 〃 | `0 50 8 ? * MON-FRI`（16:50） | `ingest_etf` | ingest-etf |
| 〃 | `0 0 9 ? * MON-FRI`（17:00） | `daily_update_plan` | data-update(update档) → plan-notify（链式合一，天然先数据后计划） |
| 〃 | `0 0 10 ? * SAT`（18:00） | `weekly_rebuild_review` | data-update(周六all档) → P4影子回测 → review（链式合一） |

入口 `faas/scheduler_handler.py::handler` 按 timer payload 分发到 `faas/jobs.py`；
Issue 评论推送（→邮件）由 `faas/gh_notify.py` 完成（原 workflow 里 github-script 的
Python 版，含同款「找到/新建跟踪 issue + 当日防重」）。任务失败会追加评论到
「⚠️ FaaS 定时任务告警」issue，替代 Actions 的失败邮件。

### 环境变量（在 fe-journey-faas 仓库的 GitHub Secrets 配置，随部署注入）

必填：`INVEST_DB_URL`（或复用 DB_HOST/DB_USER/DB_PASS 组合）、`TUSHARE_TOKEN`、
`TUSHARE_HTTP_URL`、`INVEST_GH_PAT`（fine-grained PAT：仅 invest-model Issues RW，
映射为函数内 GITHUB_TOKEN）。可选：`ACCOUNT_CASH`/`PLAN_ARGS`/`LIVE_WATCH_MENTION`
等 Variables，含义与原 workflow 相同。

FC 文件系统只读：日志与 results 导出分别经 `INVEST_LOG_DIR` / `INVEST_RESULTS_DIR`
指到 /tmp（s.yaml 已配好）；回测指标本就入库 backtest_run，latest.json 仅是便览。

### 原 workflow 的处置

7 个 workflow 的 `schedule:` 已移除（本仓库不再消耗定时额度），全部保留
`workflow_dispatch`/push 触发作手动兜底；plan-notify 仍保留「data-update 成功后
链式出计划」，手动跑 data-update 时依旧自动出计划。回滚方式：git revert 恢复
schedule 块，并在 FC 控制台禁用对应 timer 触发器即可，随时可逆。
