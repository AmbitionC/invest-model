...
- **多因子打分**：价值(EP/BP/SP)、质量(ROE/ROA/毛利)、成长(营收/净利同比)、
  动量(60/120 日，跳过近 5 日)、反转、低波、规模、低换手；截面去极值 +
  行业市值中性化 + 标准化。另有**候选因子影子机制**：北向持股变化等候选因子
  同流程算暴露、记 IC（health 可见），但不参与打分，攒够 ≥12 期 IC 评估后
  才可晋升（模型层变更流程见 `docs/model_change_proposals.md`）。
