...
def _fear_alerts(engine) -> list[tuple[str, str, str]]:
    ...
    # 文案人话口径（owner 2026-08-08 呈现契约）：不出现 E 编号与内部参数值
    if score >= thr:
        out.append((f"F:{day}:抄底窗口",
                    f"🟣 恐慌抄底窗口开启：恐慌指数 {score:.0f}（盘中 {ts}）已超 {thr:.0f}，"
                    f"转 5 分钟密控 — 今晚计划的开仓门槛会相应放宽（仅限基本面没走坏的标的）；"
                    f"盘中重点盯观察池的回踩/突破时机。分批小仓试探、不一次性买满，"
                    f"也不因更恐慌就加大单笔（历史检验：恐慌更深并不带来更好的买点）", "crit"))
    if score >= mid:
    ...
    if score >= deep:
        out.append((f"F:{day}:深度恐慌",
                    ...
                    f"更谨慎、留子弹补跌，勿因更恐慌就加大单笔（历史检验：更深的恐慌并不意味着更好的买点）",
                    "crit"))
...
def _p30_and_alert(engine, score: float, ts: str, day: str, hhmm: str) -> list[tuple[str, str, str]]:
    ...
    try:
        ...
        pass
    # 文案与计划邮件同一套人话口径（owner 2026-08-08 呈现契约：不出现 P/E 编号与术语）
    return [(f"F:{day}:{bucket}:P30杠杆",
             f"🚨🚨 加杠杆信号触发：沪深300 盘中 {px:.0f} 已低于长期中枢 "
             f"{med:.0f}（{px / med - 1:+.1%}）且市场极度恐慌（恐慌指数 {score:.0f}）——"
             f"这个组合十一年半只在 2024 年初出现过一段，其后一年指数涨了 21%~25%。"
             f"规则：只买宽基指数、借入资金不超过本金的 30%、融资成本每年不超过 6%、"
             f"由你手动执行——系统不会自动下单。信号存续期每 30 分钟重复提醒", "crit")]
