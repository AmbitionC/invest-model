    ...
    payload = {
        ...
        "bias_verdict": {
            ...
            "top3": "owner 命题「所有指数偏离度进前三＝强信号、值得中短线操作」已按完整腿"
                    ...
                    "🔴 而且它十年不响一次：104 个触发日归并后按自然月去重只剩 14 个事件，"
                    "几乎全在 2008 与 2015 两场危机，2015 年后 7 个指数总共只触发过 1 次"
                    "（2022-04 科创50）⟹ 合并置换检验 p=0.138 不显著。"
                    "做成净值后夏普 7/7 下降（−0.04~−0.60）。"
                    "涨到前三那一头方向就是错的：上证50 之后 20/60 日还涨 +9.6%/+24.6%。",
