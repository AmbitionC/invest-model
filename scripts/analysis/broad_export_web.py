...
def main() -> None:
    ...
    payload = {
        ...
        "bias_verdict": {
            "tested": "高尾分位（2026-08-02）· 低尾分位 · 排名口径两尾补齐 · 双尾前三完整腿 "
                      "· 止盈止损 sleeve · 近十年滚动窗口高卖低买"
                      "（均 2026-08-05）——五个口径的预登记验证全部未通过",
            "high_tail": "❌ 作方向信号已判死：乖离率进入全历史前 5% 分位后，未来 20 日"
            ...
            "low_tail": "❌ 低尾（跌得太深）也已判死：四条预登记判据全不过。失败机制不是「低尾不灵」，"
                        "而是低尾与恐慌抢买腿的既有价格闸几乎不相交——乖离率低尾是相对 60 日均线的"
                        "短期偏离（牛市回调也点亮），而恐慌抢买腿要求收盘低于近 5 年中位数（长期估值位置）；"
            ...
            "rank": "owner 追问「历史极值排第几、极值包不包括极大和极小」后，两个尾部都按博主原口径"
                    ...
                    "③ 低尾·因果排名 K=3/5/10：与恐慌抢买腿价格闸的共现四腿全为 0，接进该腿后 Δ年化全 +0.00。"
                    "⟹ 两个尾部的裁决都不因换排名口径而改变；高尾那个 80% 只在事后成立，"
                    "而当天你并不知道自己是不是第 1 名。",
            "top3": "owner 命题「所有指数偏离度进前三＝强信号、值得中短线操作」已按完整腿"
                    "（进场+持有+退出）在 7 个指数上验完（2026-08-05，"
            ...
            "tp_sl": "owner 追问「带好止盈止损，胜率是不是挺好」已验完（2026-08-05）——"
            ...
            "rolling10y": "owner 指定的最后一个口径——「近十年滚动窗口前四，高的卖、低的买，"
                          "单纯算这个策略本身」（2026-08-05）——同样未通过，七腿超额 "
                          ...
                          "是两种不兼容的尺度。",
            "wiring": "乖离率已在五个不同口径上全部验证失败（分位高尾 · 分位低尾 · "
                      "全历史排名双尾 · 排名+止盈止损 · 近十年滚动排名双尾状态机），"
