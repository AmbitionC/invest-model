...
方法：逐仓事件驱动模拟(同 E1 口径)。入场=投顾 long。四臂只改 MA20 处理，硬止损 -8%
  四臂始终兜底(真下跌不豁免)：
    A strict  破MA20即清(原现状)
    B grace   建仓 N 日内不启用 MA20 清仓(新仓宽限)
    C buffer  收盘破 MA20 超 x% 才清(幅度缓冲)
    D trim    生产 P10 语义逐字镜像：未盈利破MA20只减半一次；盈利仓仅「新鲜破位」
              (昨在线上今跌破)才清——缓冲仓在线下涨回成本不算破位事件
              (与 risk.evaluate_holding 的 ma20_unprofit_trim 分支一致)
...
HARD_STOP = 0.08
PROFIT_GATE = 0.0     # 与 RiskConfig.ma20_profit_gate 默认一致：close/cost-1 ≥ 此值算“盈利”
GRACE_DAYS = 5
BUFFER_PCT = 0.02
...
def _sim(closes: pd.Series, entry_date: str, mode: str) -> dict | None:
    ...
    exit_i = last_i
    ma20_hit = False
    prev_below = bool(np.isfinite(ma20[ei]) and vals[ei] < ma20[ei])  # 新鲜破位判定（D 臂）
    for i in range(ei + 1, last_i + 1):     # 建仓当日不触发
        ...
        if mode == "strict":
            ...
            # 生产 P10 语义逐字镜像（risk.evaluate_holding / replay_hold_tier）：
            # 未盈利破位→只减半一次；盈利仓仅新鲜破位(昨在线上)才清；线下转盈持有。
            profitable = c / cost - 1 >= PROFIT_GATE
            if below and profitable and not prev_below:
                trig = True
            elif below and not profitable and not trimmed:
                sell = remaining * 0.5
                realized += sell * c
                remaining -= sell
                trimmed = True
...
def run(repo: BaseRepository) -> str:
    ...
    arms = {"A 破MA20即清(现状)": "strict", "B 建仓5日宽限": "grace",
            "C 破MA20超2%才清": "buffer", "D 生产P10语义(减半+新鲜破位)": "trim"}
    ...
    if len(A) > 5:
        L.append("### 结论")
        best = None
        for k in ["B 建仓5日宽限", "C 破MA20超2%才清", "D 生产P10语义(减半+新鲜破位)"]:
